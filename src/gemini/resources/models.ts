...
import { type FinalRequestOptions, type PagePromise, type RequestOptions } from 'openai/core';
...
export class Models extends APIResource {
  ...
  async retrieve(model: string, options?: RequestOptions): Promise<Model> {
    const item: GeminiModel = await this._client.get(`/models/${model}`, options);
