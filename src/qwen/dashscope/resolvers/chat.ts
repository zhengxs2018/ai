...
export function fromChatCompletionTextMessages(
  ...
  return messages.map(message => {
    if (Array.isArray(message.content)) {
      const part = message.content.find(c => c.type === 'text') as OpenAI.ChatCompletionContentPartText;
      return { role: message.role, content: part.text } as OpenAI.ChatCompletionMessageParam;
    }
...
export function fromChatCompletionCreateParams(
  ...
  const { model, messages, raw, response_format, stream_options = {}, ...parameters } = params;
...
export function toChatCompletionFinishReason(reason?: DashscopeChat.ResponseFinish | null, stream?: boolean) {
...
function toCompletionChunk(
  ...
  if (output.choices) {
    ...
    if (finish_reason === 'tool_calls') {
      choice.finish_reason = 'tool_calls';
      choice.delta.tool_calls = message.tool_calls as OpenAI.ChatCompletionChunk.Choice.Delta.ToolCall[];
    } else {
      choice.finish_reason = toChatCompletionFinishReason(finish_reason, true);
    }
  } else {
    choice.delta.content = output.text;
    choice.finish_reason = toChatCompletionFinishReason(output.finish_reason, true);
  }
