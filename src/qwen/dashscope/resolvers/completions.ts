...
import type { DashscopeCompletions, OpenAICompletionsCompatibility } from '../types';
...
export function fromCompletionCreateParams(
  ...
  const { model, prompt, response_format, stream_options, ...parameters } = params;
...
export function toCompletionFinishReason(reason?: DashscopeCompletions.ResponseFinish | null, stream?: boolean) {
...
export function toCompletionUsage(usage: DashscopeCompletions.CompletionUsage): OpenAI.CompletionUsage {
  // hack: 部分模型不存在 total tokens?
  // 如：llama2-7b-chat-v2
  const { output_tokens, input_tokens, total_tokens = output_tokens + input_tokens } = usage;
